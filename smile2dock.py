...
def protonate_smiles(smiles, ph_min=6.4, ph_max=8.4, precision=1.0, max_variants=128):
    ...
    try:
        ...
        logging.warning("Could not protonate %s: %s", smiles, exc)
        return [smiles]  # Return original if protonation fails

def smiles_to_3d(smiles, output_base="molecule", num_confs=10, optimize=True, protonate=False, ph_min=6.4, ph_max=8.4, precision=1.0, max_variants=128):
    """Convert SMILES to multiple 3D formats and calculate properties"""
    if num_confs < 1:
        raise ValueError("num_confs must be at least 1")
    protonated_variants = [smiles]  # Always define so return is consistent
    try:
        # Protonate SMILES if requested
        if protonate:
            protonated_variants = protonate_smiles(
                smiles,
                ph_min=ph_min,
                ph_max=ph_max,
                precision=precision,
        ...
        if not protonated_variants:
            logging.error("No protonation variants generated for %s", smiles)
            return None, None, []
        ...
        try:
            ...
            logging.error("Conformer generation failed for %s:"
            "%s", smiles, exc)
            return None, None, protonated_variants

        if mol.GetNumConformers() == 0:
            logging.error("No conformers generated for %s", smiles)
            return None, None, protonated_variants
...
def batch_process(
    ...
    if reference_smiles:
        ...
        if ref_mol is None:
            logging.error("Invalid reference SMILES: %s", reference_smiles)
    else:
        ref_mol = None

    protonation_file = None
    protonation_path = os.path.join(
        output_dir,
        "protonation_states.txt",
    )

    try:
        if protonate:
            protonation_file = open(
                protonation_path,
                "w",
                encoding="utf-8",
            )
            protonation_file.write(
                "Original_SMILES\tProtonated_SMILES\tPH_Range\n"
            )
        ...
        with open(input_file, encoding="utf-8") as input_handle:
            for idx, line in enumerate(input_handle):
                ...
                if protonation_file and protonated_variants:
    ...
    if protonate:
        print(f"Protonation states saved to: {protonation_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=6.4,
        help='Minimum pH for protonation (default: 6.4)'
        )
    parser.add_argument(
        '--ph_max',
        type=float,
        default=8.4,
        help='Maximum pH for protonation (default: 8.4)'
        )
    parser.add_argument(
        '--precision',
        type=float,
        default=1.0,
        help='pKa precision factor (default: 1.0)'
        )
    parser.add_argument(
        '--max_variants',
        type=int,
        default=128,
        help='Maximum protonation variants (default: 128)'
        )
    ...
    if args.num_confs < 1:
            parser.error("--num_confs must be at least 1")

    if args.ph_min > args.ph_max:
            parser.error("--ph_min cannot be greater than --ph_max")

    if args.radius < 0:
            parser.error("--radius cannot be negative")

    if args.bits < 1:
            parser.error("--bits must be greater than zero")
